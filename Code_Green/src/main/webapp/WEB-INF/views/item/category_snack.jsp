...
<%--                                         		<c:forEach var="image" items="${imageList }"> --%>
		                    ...
		                    <div class="ps-pagination">
		                        <ul class="pagination">
		                            ...
		                            <li><%if(pageInfo.getPageNum() > pageInfo.getStartPage()) {%><a class="pageLink" href="category_snack?pageNum=${pageInfo.pageNum - 1}&searchType=${searchType }&keyword=${keyword}&sort=${sort}"><%}%><i class="icon-chevron-left"></i>Prev</a></li>
		                            <c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
		                            	<c:choose>
		                            		<c:when test="${i eq pageInfo.pageNum }"><li class="active"><a class="pageLink" href="#">${i }</a></li></c:when>
		                            		<c:otherwise><li><a class="pageLink" href="category_snack?pageNum=${i }&searchType=${searchType }&keyword=${keyword}&sort=${sort}">${i }</a></li></c:otherwise>
		                            	</c:choose>
		                            </c:forEach>
		                            <li><%if(pageInfo.getPageNum() < pageInfo.getMaxPage()) {%><a class="pageLink" href="category_snack?pageNum=${pageInfo.pageNum + 1}&searchType=${searchType }&keyword=${keyword}&sort=${sort}"><%}%>Next<i class="icon-chevron-right"></i></a></li>
		                        </ul>
		                    </div>
		                    <!-- 페이징 버튼들 끝 -->
