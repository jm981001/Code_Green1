						...
						<div id="category_h3">
						 ...
						 <nav class="ps-store-link" style="margin-top: 100px;">
							...
							<!-- 검색 기능 구현을 위한 form 태그 -->
								<form action="category_snack" method="get" style="margin-left: 20px">
                             ...
                             <!-- 페이징 버튼들 시작 -->
				                   <%PageInfo pageInfo = (PageInfo)request.getAttribute("pageInfo"); %>
				                    <div class="ps-pagination">
				                        <ul class="pagination">
				                            <li><%if(pageInfo.getPageNum() > pageInfo.getStartPage()) {%><a href="category_snack?pageNum=${pageInfo.pageNum - 1}&sort=${sort}"><%}%><i class="icon-chevron-left"></i>Prev</a></li>
				                            <c:forEach var="i" begin="${pageInfo.startPage }" end="${pageInfo.endPage }">
				                               <c:choose>
				                                  <c:when test="${i eq pageInfo.pageNum }"><li class="active"><a href="#">${i }</a></li></c:when>
				                                  <c:otherwise><li><a class="pageLink" href="category_snack?pageNum=${i }&sort=${sort}">${i }</a></li></c:otherwise>
				                               </c:choose>
				                            </c:forEach>
				                            <li><%if(pageInfo.getPageNum() < pageInfo.getMaxPage()) {%><a href="category_snack?pageNum=${pageInfo.pageNum + 1}&sort=${sort}"><%}%>Next<i class="icon-chevron-right"></i></a></li>
				                        </ul>
				                    </div>
				                <!-- 페이징 버튼들 끝 -->
